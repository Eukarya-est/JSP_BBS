...
<body>
	<%
		...
		if (request.getParameter("bb0_ID") != null){
			...
			PrintWriter script = response.getWriter();
			script.println("alert('bb0_ID is null')");
		}
		if (bb0_ID == 0) {
			PrintWriter script = response.getWriter();
			script.println("<script>");
		...
		if (!userID.equals(bb0.getUserID())) {
			...
			if(request.getParameter("bb0_Title") == null || request.getParameter("bb0_Content") == null
					|| request.getParameter("bb0_Title").equals("") || request.getParameter("bb0_Content").equals("")) {
					PrintWriter script = response.getWriter();
					script.println("<script>");
					...
					int result = bb0DAO.update(bb0_ID, request.getParameter("bb0_Title"), request.getParameter("bb0_Content"));
					if(result == -1){
						PrintWriter script = response.getWriter();
						script.println("<script>");
						script.println("alert('Modifying Failed')");
						script.println("history.back()");
						script.println("</script>");
					}
					else {
